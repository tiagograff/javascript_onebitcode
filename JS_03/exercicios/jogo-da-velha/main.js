...
function updateTitle() {
  ...
  //capturando jogador
  playerInput.innerText = playerInput.value;
...
function handleBoardClick(event) {
  ...
  const region = event.currentTarget.dataset.region; //n.n
  //transformando uma string em um array, onde tiver um ponto vai quebrar em um array
  const rowColmnPair = region.split("."); // ['a', 'b']
  //linha
  const row = rowColmnPair[0];
  //coluna
  const column = rowColmnPair[1];
